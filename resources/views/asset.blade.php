...
@section('main-content')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="update_modal_title">Update Asset </h5>
                ...
                <div class="modal-body">
                    <form id="product-update-form" method="POST" >
                        @csrf
                        <input type="text" name="id" value="{{ old("id") }}"  style="display: none" id="asset-id">
                        <div class="card-body">
                            <div class="form-group">
                                <label for="name">Name</label>
                                <input type="text" value="{{ old("name") }}"  name="name" class="form-control @error('name', "update") is-invalid invalid-update @enderror" id="asset-name" placeholder="name">
                                @error('name', "update")
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" value="{{ old("qty") }}" name="qty" class="form-control @error("qty", "update") invalid-update is-invalid @enderror " id="asset-qty" placeholder=1>
                                @error('qty', "update")
                                <span id="exampleInputEmail1-error" class="error invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <select id="condition" class="form-control select2 @error("category", "update") is-invalid invalid-update @enderror " style="width: 100%;" name="condition">
                                    <option class="select-option" value="good">good</option>
                                    <option class="select-option" value="out of order">out of order</option>
                                </select>
    ...
    <div class="modal fade" id="product-create-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="update_modal_title">Create Asset</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                    ...
                    <table id="example1" class="table table-bordered table-striped dataTable dtr-inline collapsed">
                        ...
                        @foreach($assets as $asset)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $date = new Carbon($asset->updated_at);
                                        echo $date->tz("EAT")->toDayDateTimeString()
                                    @endphp

                                </td>
                                <td>
                                    <button type="button" id="update-button" data-toggle="modal" data-target="#product-update-modal" onclick="updateProduct(objectLookup({{ $asset->id }}))" class="btn btn-block btn-primary btn-sm">Update</button>
                                    <button type="button" data-toggle="modal" data-target="#product-delete-modal" onclick="deleteProduct(objectLookup({{ $asset->id }}))" class="btn btn-block btn-danger btn-xs">Delete</button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
...
@section('extra-js')
    <script>
        ...
        function  objectLookup(id){
            for(asset of assets){
                if(parseInt(asset.id) === parseInt(id)){
                    return asset
                }
            }
        }
        function updateProduct(asset){
            const form = $("#product-update-form");
            const newUrl = updateProductUrl.concat("/", asset.id);
            form.attr("action", newUrl);
            $("#asset-name")[0].value = asset.name;
            $("#asset-qty")[0].value = asset.qty;
            $("#asset-id")[0].value = asset.id;
            $("#condition").children().each(
                function () {
                    if($(this).attr("value")=== asset.condition){
        ...
        $(function() {
            ...
            if($(".invalid-update").length>0){
                $("#condition").children().each(
                    function () {
                        if($(this).attr("value")=== "{{old("condition")}}"){
                            $(this).attr("selected", true);
                        }
                    }
                )
