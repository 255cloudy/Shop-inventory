                    ...
                    <table id="example1" class="table table-bordered table-striped dataTable dtr-inline collapsed">
                        ...
                        @foreach($products as $product)
                            <tr>
                                ...
                                <td>
                                    <button type="button" id="update-button" data-toggle="modal" data-target="#product-update-modal" onclick="insertEntry(productLookup($product->id))" class="btn btn-block btn-primary btn-sm">add</button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
    ...
    <script>
        ...
        function insertEntry(product){
            if(entryExists(product)){
                alert("Entry :"+ product.name+" already exists")
            }else {
                // push all the entries
                let data = [product.name, 0, 0, 0];
        ...
        function submitEntries(){
            ...
            $("#entry-submit-form").attr("action", "/order/create/".concat(orderNumber));
