...
@section('main-content')
    ...
    {{--    end delete modal--}}
    {{--    create order modal--}}
    <div class="modal fade" id="order-create-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="update_modal_title">Choose a Distributer </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="order-create-form" method="POST" >
                        @csrf
                        <input type="text" name="id" value="{{ old("id") }}"  style="display: none" id="entry-id">
                        <div class="card-body">
                            <div class="form-group" data-select2-id="29">
                                <label for="product">Distributer</label>
                                <select id="product" name="distributer" class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="1" tabindex="-1" aria-hidden="true">
                                    @foreach($distributers as $distributer)
                                        <option value="{{$distributer->id}}" data-select2-id="{{ $distributer->id }}">{{ $distributer->name }}</option>
                                    @endforeach
                                </select>
                                @error('create', "update")
                    ...
                    <table id="example1" class="table table-bordered table-striped dataTable dtr-inline collapsed">
                        ...
                        @foreach($orders as $order)
                            <tr>
                                ...
                                <td>
                                    <button type="button" id="update-button" data-toggle="modal" data-target="#product-update-modal" onclick="updateProduct(objectLookup({{ $order->id }}))" class="btn btn-block btn-primary btn-sm">View</button>
...
@section('extra-js')
    <script>
        ...
        $("#add-btn").click(function(){$('#order-create-modal').modal().show()})
        const create_url = "/order/";
        $("#order-create-form").attr("action", create_url);
    </script>
